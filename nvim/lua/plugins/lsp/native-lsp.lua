...
M.setup = function()
  ...
  local ok, err = pcall(function()
    ...
    if not snacks_ok then
      vim.notify('LSP: snacks.nvim not found. Proceeding without advanced notifications.', vim.log.levels.WARN)
    end
    ...
    if has_blink then
      ...
      vim.notify('LSP: blink.cmp not found. Proceeding with default capabilities.', vim.log.levels.WARN)
    end
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      if client and client.server_capabilities.documentHighlightProvider then
        local highlight_group = vim.api.nvim_create_augroup('LSP_Highlight_' .. event.buf, { clear = false })
