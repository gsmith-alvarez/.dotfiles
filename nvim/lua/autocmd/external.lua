...
vim.api.nvim_create_autocmd('BufReadPre', {
  ...
  callback = function(ev)
    ...
    -- PERFORMANCE: Use native Libuv fs_stat
    local ok, stats = pcall(vim.uv.fs_stat, ev.match)

    if ok and stats and stats.size > max_filesize then
      -- 1. Set a buffer-local flag for other plugins (like LSP and Indent guides) to respect
      vim.b[ev.buf].bigfile = true

      -- 2. Localized Degradation:
      -- Disable vim regex syntax for THIS buffer only
      vim.opt_local.syntax = ''

      -- Disable Treesitter for THIS buffer only (Neovim 0.10+ API)
